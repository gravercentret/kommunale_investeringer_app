...
with col1:
    ...
    selected_priorities = st.multiselect(
        ...
        options=[
            ...
            2,
            3,
        ],  # Priority options (None: Almindelig, 1: yellow, 2: orange, 3: red)
...
if search_query:
    ...
    st.markdown(
        f"***Antal kommuner/regioner med problematiske investeringer:*** \n **{filtered_df.select(pl.col("Kommune").n_unique()).to_numpy()[0][0]}**"
    )
...
with col_count:
    ...
    # Filter the original dataframe based on the top 10 municipalities
    filtered_df_top_10_count = filtered_df.filter(
        pl.col("Kommune").is_in(top_10_kommuner_count_list)
    )
